...
get_prop() {
  ...
  for f in $S/build.prop $S/config.prop; do
    if [ -e "$f" ]; then
...
set_pathmap() {
  if [ "$android_sdk" == "$supported_sdk_v30" ]; then
    SYSTEM="$S/system_ext"
    test -d $SYSTEM/etc/default-permissions || mkdir -p $SYSTEM/etc/default-permissions
    test -d $SYSTEM/etc/preferred-apps || mkdir -p $SYSTEM/etc/preferred-apps
    test -d $SYSTEM/etc/sysconfig || mkdir -p $SYSTEM/etc/sysconfig
    test -d $SYSTEM/lib || mkdir -p $SYSTEM/lib
    test -d $SYSTEM/lib64 || mkdir -p $SYSTEM/lib64
...
restoredirTMP() {
  ...
  TMP_LIB="
    ...
    $TMP/lib/libfrsdk.so
    $TMP/lib/libsketchology_native.so"
  ...
  TMP_LIB64="
    ...
    $TMP/lib64/libsketchology_native.so"
...
conf_addon_backup() {
  if [ -d $TMP/priv-app/GoogleServicesFramework ]; then
    ui_print "BackupTools: BiTGApps backup done";
  else
    ui_print "BackupTools: Failed to create BiTGApps backup";
    rm -rf $TMP/addon.d/90-bitgapps.sh
  fi;
}

# Confirm that restore is done
conf_addon_restore() {
  if [ -d $SYSTEM/priv-app/GoogleServicesFramework ]; then
    ui_print "BackupTools: BiTGApps restore done";
  else
